...
from utils_v1 import quat2axisangle, axisangle2quat, get_answer_from_server


# 从robot_state.json获取机器人状态序列
def get_actions_to_replay(ep_path):
    ...
    # 6D eef + 1D lift
    replay_actions = np.concatenate([list_eef_pose, list_grip], axis=-1)
...
class VRArm(Node):
    ...
    use_replay = True
    src_data_dir = "/airbot_vr_py/dataset_1125/episode_31"
    remote_policy_host = "10.120.17.111"
    remote_policy_port = 8000
    replan_steps = 12
    speedup_act = 1
    prompt = "Which of the following object is on the nearest table, a bottle or a blue-and-white carton or a banana peel? You should focus on the table within reach."
    # task = "Pick up the blue carton or the banana peel or the bottle"
    qwen3vl_server_url = "http://10.120.17.111:8988"
    keyword_2_task = {
        "carton": "pick up the carton",
        "banana peel": "pick up the banana peel",
        "bottle": "pick up the bottle",
    }
    ...
    def __init__(self, freq: int, record_freq: int,
        ...
        self.writer = AsyncEpisodeWriter(
            data_root="./rollout",
            camera_config=[
                {"name": "main", "has_rgb": True, "has_depth": False, "width": 1280, "height": 720},
        ...
        while self.main_color is None or self.wrist_color is None:
            time.sleep(0.1)
        
        # main_rgb = bgr2jpgbytes(self.main_color)
        wrist_rgb = bgr2jpgbytes(self.wrist_color)
        # count the frequency of each keyword in the prompt
        keyword_counts = {}
        for keyword in self.keyword_2_task.keys():
            keyword_counts[keyword] = 0
        for _ in range(3):
            response = get_answer_from_server(wrist_rgb, self.prompt, server_url=self.qwen3vl_server_url)
            output_text = response['output_text'][0]
            print_green(f"output_text: {output_text}")
            print_green(f"[Qwen3vl latency]: {response['generation_time']}s")
            for keyword in self.keyword_2_task.keys():
                keyword_counts[keyword] += output_text.count(keyword)
            print_green(f"keyword_counts: {keyword_counts}")
        # find the keyword with the maximum count
        self.keyword = max(keyword_counts, key=keyword_counts.get)
        print_green(f"self.keyword: {self.keyword}")
        self.task = self.keyword_2_task[self.keyword]
        print_green(f"self.task: {self.task}")

        self.action_plan = collections.deque()
        if self.use_replay:
            actions_to_replay = get_actions_to_replay(self.src_data_dir)
            self.action_plan.extend(actions_to_replay)
        else:
            self.remote_policy = _websocket_client_policy.WebsocketClientPolicy(
                self.remote_policy_host, self.remote_policy_port)
            input_data = {
                "rgb_main": np.zeros((224, 224, 3), dtype=np.uint8),
                "rgb_wrist": np.zeros((224, 224, 3), dtype=np.uint8),
                "state": np.zeros(13),
                "prompt": self.task,
            }
            # dummy run to warm-up
            action_chunk = self.remote_policy.infer(input_data)["actions"]
    ...
    def rolloutProcess(self):
        ...
        if now - self._last_record_time >= self.record_interval:
            ...
            if self.main_color is None or self.wrist_color is None:
                return
            ...
            if len(self.action_plan) == 0:
                if self.use_replay:
                    ...
                    joint_pos = np.array(robot_state['joint_pos'])
                    eef_pos = np.array(robot_state["eef_pos"])
                    eef_rot = quat2axisangle(np.array(robot_state["eef_quat"]))
                    grip_state = np.array([0]) if robot_state["gripper"]==0.07 else np.array([1])
                    state = np.concatenate([joint_pos, eef_pos, eef_rot, grip_state], axis=0)
                    input_data = {
                        "rgb_main": convert_image(camera_data["main"]["rgb"]),
                        "rgb_wrist": convert_image(camera_data["wrist"]["rgb"]),
                        "state": state,
                        "prompt": self.task
                    }
                    ...
                    print_yellow(f'[Remote Inference Latency]: {infer_time}s')
                    assert (
                        len(action_chunk) >= self.replan_steps
                    ), f"len(action_chunk)={len(action_chunk)} < self.replan_steps={self.replan_steps}"
                    tmp = action_chunk[: self.replan_steps]
                    self.action_plan.extend(tmp)

            # get action
            next_act_step_size = min(self.speedup_act, len(self.action_plan))
            ...
            eef_act[:3, 3] = eef_pos
            grip_act = action[6]
            self.tctr_gripper = 0 if grip_act > 0.5 else 0.07
...
if __name__ == "__main__":
    ...
    try:
        ...
        vr_arm_node.reset()
        vr_arm_node.arm.update_eef(0.07)
